...
func (s *Ratelimits) DeleteOverride(ctx context.Context, request operations.DeleteOverrideRequestBody, opts ...operations.Option) (*operations.DeleteOverrideResponse, error) {
	hookCtx := hooks.HookContext{
		Context:        ctx,
	...
	opURL, err := url.JoinPath(baseURL, "/v1/ratelimits.deleteOverride")
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	res := &operations.DeleteOverrideResponse{
		HTTPMeta: components.HTTPMetadata{
			Request:  req,
			Response: httpRes,
		},
	...
	case httpRes.StatusCode == 200:
		...
		case utils.MatchContentType(httpRes.Header.Get("Content-Type"), `application/json`):
			...
			var out operations.DeleteOverrideResponseBody
			if err := utils.UnmarshalJsonFromResponseBody(bytes.NewBuffer(rawBody), &out, ""); err != nil {
				return nil, err
			}
